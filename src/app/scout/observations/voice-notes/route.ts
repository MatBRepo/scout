...
async function supabaseFromCookies() {
  const cookieStore = await cookies() // Next 15: cookies() is a Promise
  return createServerClient(
    ...
    process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!,
    {
      cookies: {
        ...
        // Provide no-ops or delegate to cookieStore as needed
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (durationSec === undefined || durationSec === null || Number.isNaN(Number(durationSec))) {
      missing.push("durationSec")
    }
    if (missing.length) {
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!observationId) {
      return NextResponse.json({ ok: false, error: "missing fields: observationId" }, { status: 400 })
    }
