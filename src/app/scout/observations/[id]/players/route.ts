...
export async function POST(req: Request, { params }: { params: { id: string } }) {
  ...
  const player_entry_id: string | undefined = body.player_entry_id

  // exactly one of player_id or player_entry_id
  if ((!player_id && !player_entry_id) || (player_id && player_entry_id)) {
    return NextResponse.json({ error: "Provide exactly one of player_id or player_entry_id" }, { status: 400 })
  }

  // duplicate guard
  let dup = supabase.from("observation_players")
    .select("id")
    .eq("observation_id", observationId)
  if (player_id) dup = dup.eq("player_id", player_id)
  if (player_entry_id) dup = dup.eq("player_entry_id", player_entry_id)
...
// Optional list endpoint used by clients if needed
