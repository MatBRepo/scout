    ...
    <div className="grid gap-6">
      <div className="flex items-center gap-4">
        <div className="w-16 h-16 rounded-full overflow-hidden bg-muted grid place-items-center text-xs">
          {avatarUrl ? <Image src={avatarUrl} alt="Avatar" width={64} height={64} /> : "No avatar"}
        </div>
        <div className="text-sm text-muted-foreground">Recommended: square image (e.g. 256×256).</div>
      </div>
      ...
      <div className="grid gap-3">
        ...
        <Input id="avatar_url" value={avatarUrl} onChange={(e) => setAvatarUrl(e.target.value)} placeholder="https://…/avatar.png" />
      </div>
      ...
      <div className="flex gap-2">
        <Button disabled={isPending} onClick={onSave}>{isPending ? "Saving…" : "Save changes"}</Button>
      </div>
    </div>
  )
}
